...
# class ACmix [https://arxiv.org/abs/2111.14556]
...
"""
Channel Attention Module​​: Maintains channel dimension while compressing spatial dimensions. Focuses on meaningful information in the input image.
"""
class ChannelAttention(nn.Module):
    # Channel-attention module https://github.com/open-mmlab/mmdetection/tree/v3.0.0rc1/configs/rtmdet
    def __init__(self, channels: int) -> None:
        super().__init__()
        self.pool = nn.AdaptiveAvgPool2d(1)  # (b,c,w,h) => (b,c,1,1)
        self.fc = nn.Conv2d(channels, channels, 1, 1, 0, bias=True)  # 1x1 conv
        self.act = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return x * self.act(self.fc(self.pool(x)))  # Multiply attention weights

"""
Spatial Attention Module​​: Maintains spatial dimensions while compressing channel dimension. Focuses on positional information
"""
class SpatialAttention(nn.Module):
    # Spatial-attention module
    def __init__(self, kernel_size=7):
        ...
        assert kernel_size in (3, 7), 'kernel size must be 3 or 7'
        padding = 3 if kernel_size == 7 else 1
        self.cv1 = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)  # Fuses channel-reduced features
        self.act = nn.Sigmoid()

    def forward(self, x):
        # Concatenate mean & max along channel axis: (b,2,w,h)
        return x * self.act(self.cv1(torch.cat(
            [torch.mean(x, 1, keepdim=True),  # Channel average (b,1,w,h)
             torch.max(x, 1, keepdim=True)[0]], 1)))  # Channel max (b,1,w,h)

"""
CBAM Module​​: Sequential application of channel & spatial attention
"""
class CBAM(nn.Module):
    # Convolutional Block Attention Module
    def __init__(self, c1, kernel_size=7):  # ch_in, kernels
        super().__init__()
        self.channel_attention = ChannelAttention(c1)  # Process channels first
        self.spatial_attention = SpatialAttention(kernel_size)  # Then spatial

    def forward(self, x):
        return self.spatial_attention(self.channel_attention(x))  # Sequential processing
...
"""SPD-Conv"""
# https://github.com/LabSAINT/SPD-Conv/blob/main/YOLOv5-SPD/models/common.py
# https://blog.csdn.net/qq_41739364/article/details/136255986
class SPD(nn.Module):
    '''
    The role of the SPD layer is to flatten the local spatial information (in blocks of block_size x block_size) of the input feature map into the channel dimension, thereby:

        - Reducing spatial resolution: The spatial dimensions are reduced to 1/block_size of their original size.
        - Increasing the number of channels: The number of channels becomes block_size² times the original number.

    This transformation preserves local spatial information while adjusting the ratio between channel and spatial dimensions. It is commonly used in lightweight models or feature fusion tasks.
    '''
    ...
    #         f"The spatial dimensions must be divisible by the block_size. This ensures that: H: {H}, W: {W}"
