...
class KernelSpatialModulation_Global(nn.Module):
    ...
    def get_channel_attention(self, x):
        # 处理小尺寸输入的情况
        channel_fc_out = self.channel_fc(x)

        # 检查输入尺寸
        if x.size(0) == 1 and (x.size(2) <= 1 or x.size(3) <= 1):
            # 对于小批量和小空间维度的情况，使用固定尺寸
            if self.spatial_freq_decompose:
                channel_fc_out = channel_fc_out.view(x.size(0), 1, 1, -1, 1, 1)
            else:
                channel_fc_out = channel_fc_out.view(x.size(0), 1, 1, -1, 1, 1)
        else:
            # 原始处理逻辑
            channel_fc_out = channel_fc_out.view(x.size(0), 1, 1, -1, x.size(-2), x.size(-1))

        if self.act_type == 'sigmoid':
            channel_attention = torch.sigmoid(channel_fc_out / self.temperature) * self.att_multi
        elif self.act_type == 'tanh':
            channel_attention = 1 + torch.tanh_(channel_fc_out / self.temperature)
        else:
    ...
    def get_filter_attention(self, x):
        # 处理小尺寸输入的情况
        filter_fc_out = self.filter_fc(x)

        # 检查输入尺寸
        if x.size(0) == 1 and (x.size(2) <= 1 or x.size(3) <= 1):
            # 对于小批量和小空间维度的情况，使用固定尺寸
            filter_fc_out = filter_fc_out.view(x.size(0), 1, -1, 1, 1, 1)
        else:
            # 原始处理逻辑
            filter_fc_out = filter_fc_out.view(x.size(0), 1, -1, 1, x.size(-2), x.size(-1))

        if self.act_type == 'sigmoid':
            filter_attention = torch.sigmoid(filter_fc_out / self.temperature) * self.att_multi
        elif self.act_type == 'tanh':
            filter_attention = 1 + torch.tanh_(filter_fc_out / self.temperature)
        else:
            raise NotImplementedError
...
if __name__ == '__main__':
    ...
    # 创建一个输入张量，形状为 (1, 3, 640, 640)
    input_tensor = torch.randn(1, 3, 640, 640)

    # 执行前向传播
    output_tensor = fdconv(input_tensor)
