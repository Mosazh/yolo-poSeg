...
from ultralytics.utils.ops import segments2boxes, seg2xywh
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    ...
                    keypoints = np.array(
                        [
                            np.array(x[5 : nkpt * ndim + 5], dtype=np.float32)
                            if int(x[0]) in kpt_names.keys()
                            else np.zeros((nkpt * ndim), dtype=np.float32)
                            for x in lb
                        ],
                        dtype=np.float32,
                    )  # (cls, x_center, y_center, width, height, pxy1..., xy1...)

                    segments =[
                        np.array(x[5 + nkpt * ndim:], dtype=np.float32).reshape(-1, 2)
                        if int(x[0]) in kpt_names.keys()
                        else np.array(x[1:], dtype=np.float32).reshape(-1, 2)
                        for x in lb
                    ]  # (cls, pxy1..., xy1...)
...
def verify_image_poseg_label_Insulate(args):
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, kpt_names = args
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                # (cls, x_center, y_center, width, height, pxy1..., xy1...)
                if keypoint and any(len(x) > (5 + nkpt * ndim) for x in lb):  # is segment and keypoint
                    classes = np.array([x[0] for x in lb], dtype=np.float32)
                    keypoints = np.array(
                        [np.array(x[5 : nkpt * ndim + 5], dtype=np.float32) for x in lb],
                        dtype=np.float32,
                    )

                    segments = [np.array(x[5 + nkpt * ndim:], dtype=np.float32).reshape(-1, 2) for x in lb]
                    # TODO: check original box of segments classes
                    Oboxes = segments2boxes(segments)
                    boxes = (
                        [
                            np.array(x[1:5], dtype=np.float32)
                            if int(x[0]) in kpt_names.keys()
                            else seg2xywh(np.array(x[5 + nkpt * ndim:], dtype=np.float32).reshape(-1, 2))
                            for x in lb
                        ]
                    )

                    lb = np.concatenate((classes.reshape(-1, 1), boxes, keypoints), 1)  # (cls, xywh)
