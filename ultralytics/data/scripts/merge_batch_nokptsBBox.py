...
def process_single_pair(seg_lines, pose_lines):
    ...
    for pl in pose_lines:
        ...
        if len(parts) >= 5:
            try:
                bbox, kpts = parse_pose_line(parts)
                pose_data.append((bbox, kpts))
            except:
                continue
    ...
    for seg_line in seg_lines:
        ...
        try:
            ...
            # 寻找最佳匹配pose
            for pose_bbox, kpts in pose_data:
                current_iou = compute_iou(seg_bbox, pose_bbox)
            ...
            # 构造新行
            if best_kpts:
                new_line = [seg_parts[0]] + best_kpts + seg_parts[1:]
