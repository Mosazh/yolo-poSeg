...
class SlidingWindowPredictor:
    ...
    def predict_and_save(self, image_path, output_dir):
        ...
        # Records for class 0 and class 1, class2
        class0_records = []
        class1_records = []
        class2_records = []
        ...
        for idx, (x1, y1, x2, y2) in enumerate(tqdm(windows, desc="Sliding Windows")):
            ...
            for i, box in enumerate(res.boxes):
                ...
                elif cls_idx == 2 and masks is not None:
                    mask_i = masks[i]
        ...
        # Save class 2 convex hulls to JSON
        json2_path = os.path.join(output_dir, f"{base_name}_class2.json")
        with open(json2_path, 'w') as jf:
            json.dump(class2_records, jf, indent=2)
            print(f"Saved class1 JSON: {json2_path} (objects: {len(class2_records)})")
